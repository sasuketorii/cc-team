...
1. **スクリプトの作成**
   - 適切なディレクトリに配置（`scripts/`, `tests/`, `.claude/commands/`）
   - 実行権限を付与: `chmod +x your-script.sh`

2. **COMMANDS.mdの更新**
   - `/Users/sasuketorii/CC-Team/CCTeam/COMMANDS.md`を更新
   - 適切なセクションに追加
   - 説明、引数、出力例を明記

3. **ドキュメントの更新**
   - 必要に応じて`scripts/README.md`も更新
   - 関連するスクリプトとの連携を記載

### コマンド一覧の参照

Claudeは以下のファイルを読み込むことで、利用可能なすべてのコマンドを把握できます：
```
@CCTeam/COMMANDS.md
```
