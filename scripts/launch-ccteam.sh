...
ユーザーから「requirementsを読み込んで作業を開始してください」などの明確な指示があったら、requirements/ フォルダ内の要件定義を全て読み込んで、プロジェクトの全体像を把握し、適切なタスクに分解してWorkerに割り当ててください。
...
⚠️ 重要: 現在は待機モードです。BOSSから明確な指示があるまで、自動的に作業を開始しないでください。
